...
- `python/packages/core/agent_framework/hil_workflow.py` implements the shared scaffolding: SQL connectors for DuckDB/SQLite/Postgres with approval-gated tools, a toy retriever, and an orchestrator that builds Planner → SQL → RAG → Reasoning → Response agents.
- `python/samples/demos/hil_workflow/hil_workflow.py` shows how to wire the orchestrator with a SQLite demo (swap connectors for DuckDB/Postgres) and stream events end-to-end for UI integration.
