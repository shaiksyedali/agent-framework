...
from framework.tools.csv_tools import CsvTools
...
class WorkflowBuilder:
    ...
    def _get_reranker(self):
        ...
        try:
            ...
            # Prefer bge-reranker if available; stay configurable
            model_name = os.getenv("BGE_RERANKER_MODEL", "BAAI/bge-reranker-large")
            top_n = int(os.getenv("RERANK_TOP_N", "15"))
            self._reranker = SentenceTransformerReranker(model=model_name, top_n=top_n)
        except Exception as e:
            print(f"Warning: sentence-transformers reranker unavailable ({e}); continuing without reranker.")
            self._reranker = None
        return self._reranker
    ...
    def build_agent(self, config: AgentConfig, data_sources: Dict[str, Any]) -> Agent:
        ...
        for ds_id in (config.data_sources or []):
            ...
            if ds.type == "database":
                ...
                if not agent_knowledge:
                    ...
                    try:
                        mmr_diversity = float(os.getenv("KNOWLEDGE_MMR_DIVERSITY", "0.3"))
                    except Exception:
                        mmr_diversity = 0.3
                    try:
                        mmr_limit_env = os.getenv("KNOWLEDGE_MMR_LIMIT", "0")
                        mmr_limit = int(mmr_limit_env) if mmr_limit_env is not None else 0
                    except Exception:
                        mmr_limit = 0
                    ...
                    try:
                        ...
                        filtered_kwargs = {k: v for k, v in knowledge_kwargs.items() if k in supported}
                    except Exception:
                        filtered_kwargs = knowledge_kwargs

                    agent_knowledge = Knowledge(**filtered_kwargs)
                    # Attach reranker if constructor did not accept it
                    if knowledge_kwargs.get("reranker") and not getattr(agent_knowledge, "reranker", None):
                        try:
                            agent_knowledge.reranker = knowledge_kwargs["reranker"]  # type: ignore[attr-defined]
                        except Exception:
                            pass
        ...
        # Step chaining reinforcement
        extra_instructions += "\n\n**Step Chaining**: Always consume prior step outputs from context keys provided (e.g., {last_step_output}, {last_step_metrics}, or explicit keys like {analysis_result}). Do NOT ignore earlier results; incorporate them into your analysis."
        ...
        if file_data_sources:
            ...
            # Add CSV utilities when tabular files present
            if any(str(p or "").lower().endswith((".csv", ".xlsx", ".xls")) for p in available_files):
                agent_tools.append(CsvTools())
            ...
            # CSV/XLSX guardrail: force header/schema inspection
            if any(str(p or "").lower().endswith((".csv", ".xlsx", ".xls")) for p in available_files):
                extra_instructions += (
                    "\n\n**Tabular (CSV/XLSX) Guardrails**:\n"
                    "- Before answering anything about columns, metrics, or time periods, inspect the headers from the tabular files. Do NOT guess column names.\n"
                    "- Use `csv_tools.list_columns` and `csv_tools.sample_rows` to inspect schema and data. If needed, read a small sample to confirm header spelling.\n"
                    "- For aggregations, use `csv_tools.aggregate_sum` with explicit group_by/value/date filters instead of guessing.\n"
                    "- In your reasoning and answers, echo the exact column names you used."
                )
