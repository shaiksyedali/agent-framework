        ...
        if "visualizations" in data:
            ...
            for vis in vis_list:
                ...
                if isinstance(vis, dict):
                    ...
                    # Check for invalid type='table'
                    if vis.get("type") == "table":
                        # Case A: Content field (Markdown)
                        table_content = vis.get("content")
                        if table_content and isinstance(table_content, str):
                            data["content"] += f"\n\n{table_content}"
                        
                        # Case B: Columns and Data fields
                        elif "columns" in vis and "data" in vis:
                            cols = vis["columns"]
                            rows = vis["data"]
                            if isinstance(cols, list) and isinstance(rows, list):
                                md_table = f"\n\n### {vis.get('title', 'Table')}\n\n"
                                md_table += "| " + " | ".join([str(c) for c in cols]) + " |\n"
